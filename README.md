...
| [yueban-docs-freshness-audit](skills/yueban-docs-freshness-audit/SKILL.md) | Audit AGENTS.md/README.md/`docs/`/`skills/` markdown (root and each active submodule) for staleness (dangling references, contradictions, broken links, cross-doc inconsistency, stale command/paths, etc.) and fix issues directly | Only on explicit invocation (`/yueban-docs-freshness-audit`) |
