...
| [yueban-docs-prune-historical-comments](skills/yueban-docs-prune-historical-comments/SKILL.md) | Scan and remove "how this used to be, then changed to" historical-narrative comments/doc passages that no longer guide current behavior | Only on explicit invocation, or when the user says this kind of historical-narrative comment is unnecessary |
...
| [yueban-loop-openspec](skills/yueban-loop-openspec/SKILL.md) | Turn a Goal into an unattended OpenSpec loop (explore → propose → apply → verify → archive) | Claude Code only; explicit invocation only (`/yueban-loop-openspec <goal>`, or `/loop /yueban-loop-openspec <goal>` for unattended runs) |
